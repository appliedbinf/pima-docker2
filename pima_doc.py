...
for i in opts.keys():
    if i in [
            'ont-fast5','ont-fastq','genome','plasmid-database',
            'amr-database','inc-database','reference-dir','reference-genome','mutation-regions']:
...
def calldocker(opts):
    command_string = ' '.join(f'{key}={value}' for key, value in opts.items())
